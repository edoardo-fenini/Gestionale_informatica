...
if($method==="GET"){
    $query = "SELECT * FROM impieghi WHERE 1=1";
    $params=[]; $types="";
...
elseif($method==="POST"){
    ...
    $stmt=$conn->prepare("INSERT INTO impieghi(".implode(",",$keys).") VALUES($ph)");
...
elseif($method==="DELETE"){
    $input=json_decode(file_get_contents("php://input"),true);
    if(!$input || !isset($input['id_impiego'])){echo json_encode(["error"=>"ID mancante"]); exit;}
    $stmt=$conn->prepare("DELETE FROM impieghi WHERE id_impiego=?");
    $stmt->bind_param("i",$input['id_impiego']);
