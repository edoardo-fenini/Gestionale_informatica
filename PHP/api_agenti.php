...
if($method==="GET"){
    $query = "SELECT * FROM agenti WHERE 1=1";
    $params=[]; $types="";
...
elseif($method==="POST"){
    ...
    $stmt=$conn->prepare("INSERT INTO agenti(".implode(",",$keys).") VALUES($ph)");
...
elseif($method==="DELETE"){
    $input=json_decode(file_get_contents("php://input"),true);
    if(!$input || !isset($input['id_agente'])){echo json_encode(["error"=>"ID mancante"]); exit;}
    $stmt=$conn->prepare("DELETE FROM agenti WHERE id_agente=?");
    $stmt->bind_param("i",$input['id_agente']);
